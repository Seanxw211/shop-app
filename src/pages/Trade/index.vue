  ...
  <div class="trade-container">
    ...
    <div class="content">
      ...
      <div
        ...
        v-for="address in addressInfe"
        :key="address.id"
      >
        <span class="username" :class="{ selected: address.isDefault == 1 }">
          {{ address.consignee }}
        </span>
        <p @click="changeDefault(address, addressInfe)">
...
export default {
  ...
  computed: {
    ...mapState({
      addressInfe: (state) => state.trade.address,
      orderInfo: (state) => state.trade.orderInfo,
    }),
    // 选中的地址
    userDefaultAddress() {
      // find 查找数组当中符合条件的元素返回，作为最终结果
      return this.addressInfe.find((item) => item.isDefault == 1) || {};
    },
  },
  mounted() {
  ...
  methods: {
    // 修改地址
    changeDefault(address, addressInfe) {
      // 先让全部 isDefault 为 0
      addressInfe.forEach((item) => (item.isDefault = 0));
      // 点击的 为 1
      address.isDefault = 1;
    },
    // 提交订单
    async submitOrder() {
      ...
      let data = {
        ...
        paymentWay: "ONLINE",
        orderComment: this.msg, //收件人的留言
        orderDetailList: this.orderInfo.detailArrayList, //购买的商品清单
      };

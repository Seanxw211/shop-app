        ...
        <div class="InfoWrap">
          ...
          <div class="choose">
            ...
            <div class="cartWrap">
              ...
              <div class="add">
                <!-- 普通的路由跳转是从A到B
                      这里的路由跳转之前要发送请求，把所挑选的产品信息通过请求通知服务器，服务器进行相应的存储
                 -->
...
export default {
  ...
  methods: {
    ...
    // 加入购物车
    async addShopcart() {
      // 1、发请求---将产品通知服务器 加入数据库
      /*
        因为action请求有async修饰 所以会有一个promite的返回值 
        用result接收来判断请求是否成功
         */
      try {
        // 成功 进行跳转
        ...
        // 进行路由跳转
        // 将路由信息带到下一个页面
        // 如果只有一些简单的信息  可以通过query形式给路由组件传递高期
        // 如果是复杂的数据，如数组、对象等，通过会话存储（不持久 会话结束就消失）
        // 会话存储 存的是字符串 不能直接存对象
